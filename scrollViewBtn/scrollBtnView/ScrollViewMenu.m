...
- (void)layoutSubviews {
  ...
  _scrollView.frame = CGRectMake(0, 0, CGRectGetWidth(self.bounds),
                                 CGRectGetHeight(self.bounds));
}
- (void)createMenuItems:(NSArray*)aItemsArray {
  //清除scrollView上的其他View
  ...
  for (NSMutableDictionary* lDic in aItemsArray) {
    ...
    BtnView* vButton = [[BtnView alloc]
        initWithFrame:CGRectMake(menuWidth,
                                 _scrollView.frame.size.height / 2.0 -
                                     vButtonWidth / 2.0,
                                 vButtonWidth, vButtonWidth)
             withDict:lDic];
    [vButton setTag:i];
  ...
  [_scrollView
      setContentSize:CGSizeMake(menuWidth, CGRectGetHeight(self.bounds))];
...
- (void)moveScrolViewWithIndex:(NSInteger)aIndex {
  ...
  if (vButtonOrigin >= CGRectGetWidth(self.bounds)) {
    if ((vButtonOrigin + CGRectGetWidth(self.bounds) / 2.0) >=
        _scrollView.contentSize.width) {
      [_scrollView
          setContentOffset:CGPointMake(_scrollView.contentSize.width - 180,
    ...
    if (vMoveToContentOffset > 0) {
      [_scrollView setContentOffset:CGPointMake(vMoveToContentOffset,
                                                _scrollView.contentOffset.y)
                           animated:YES];
    }
  } else {
